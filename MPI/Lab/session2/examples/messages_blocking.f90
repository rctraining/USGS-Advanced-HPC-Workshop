...
CONTAINS
    ...
    SUBROUTINE SWAP_DOUBLE()
        ...
        IF (my_rank .eq. 0) THEN 
            ...
            Call MPI_Recv(my_val, 1, MPI_DOUBLE_PRECISION, source, & 
                mtag1, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
            WRITE(output_unit,'(a,F4.1)')'Rank 0; post-receive; my_val = ', my_val

            !Once we have received, we send to rank 1
            dest = 1
            Call MPI_Send(send_val, 1, MPI_DOUBLE_PRECISION, dest, & 
                mtag2, MPI_COMM_WORLD,ierr)
        ENDIF
        ...
        IF (my_rank .eq. 1) THEN ! highest rank does not send
            ...
            dest = 0
            Call MPI_Send(send_val, 1, MPI_DOUBLE_PRECISION, dest, & 
                mtag1, MPI_COMM_WORLD, ierr)

            source = 0 
            WRITE(output_unit,'(a,F4.1)')'Rank 1;  pre-receive; my_val = ', my_val
            Call MPI_Recv(my_val, 1, MPI_DOUBLE_PRECISION, source, & 
                mtag2, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
            WRITE(output_unit,'(a,F4.1)')'Rank 1; post-receive; my_val = ', my_val
    ...
    SUBROUTINE SWAP_INTEGER()
        ...
        IF (my_rank .eq. 0) THEN 
            ...
            Call MPI_Recv(my_val, 1, MPI_INTEGER, source, & 
                mtag1, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
            WRITE(output_unit,'(a,i0)')'Rank 0; post-receive; my_val = ', my_val

            !Once we have received, we send to rank 1
            dest = 1
            Call MPI_Send(send_val, 1, MPI_INTEGER, dest, & 
                mtag2, MPI_COMM_WORLD, ierr)
        ENDIF
        ...
        IF (my_rank .eq. 1) THEN ! highest rank does not send
            ...
            Call MPI_Recv(my_val, 1, MPI_INTEGER, source, & 
                mtag2, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
            WRITE(output_unit,'(a,i0)')'Rank 1; post-receive; my_val = ', my_val
    ...
    SUBROUTINE SWAP_MULTI_DOUBLE()
        ...
        IF (my_rank .eq. 0) THEN 
            ...
            Call MPI_Recv(my_vals(3), 2, MPI_DOUBLE_PRECISION, source, & 
                mtag1, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
            WRITE(output_unit,'(a,4F4.1)')'Rank 0; post-receive; my_vals = ', my_vals

            !Once we have received, we send to rank 1
            dest = 1
            Call MPI_Send(send_buffer, 2, MPI_DOUBLE_PRECISION, dest, & 
                mtag2, MPI_COMM_WORLD, ierr)
        ENDIF
        IF (my_rank .eq. 1) THEN ! highest rank does not send
            ...
            Call MPI_Recv(my_vals(3), 2, MPI_DOUBLE_PRECISION, source, & 
                mtag2, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
            WRITE(output_unit,'(a,4F4.1)')'Rank 1; post-receive; my_vals = ', my_vals
