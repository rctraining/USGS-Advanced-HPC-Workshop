...
CONTAINS
    ...
    SUBROUTINE GHOST_ZONE_COMM()
        ...
        IF (my_rank .gt. 0) THEN  ! rank zero does not receive
            ...
            source = my_rank-1
            tag = my_rank-1
            Call MPI_Recv(var(my_imin-1), 1, MPI_DOUBLE_PRECISION, source, & 
        ...
        IF (my_rank .lt. (ncpu-1)) THEN ! highest rank does not send
            ...
            dest = my_rank+1
            tag  = my_rank
            Call MPI_Send(rbval, 1, MPI_DOUBLE_PRECISION, dest,tag, &
                & MPI_COMM_WORLD,ierr)
        ENDIF
        ...
        IF (my_rank .lt. (ncpu-1)) THEN ! highest rank does not receive
            ...
            source = my_rank+1
            tag    = my_rank+1
            Call MPI_Recv(var(my_imax+1), 1, MPI_DOUBLE_PRECISION, source, & 
                tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE,ierr)
        ENDIF
        IF (my_rank .gt. 0) THEN ! rank zero does not send
            ...
            dest = my_rank-1
            tag  = my_rank
            Call MPI_Send(lbval, 1, MPI_DOUBLE_PRECISION, dest,tag, &
                & MPI_COMM_WORLD,ierr)
        ENDIF
