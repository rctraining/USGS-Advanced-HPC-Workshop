...
CONTAINS
    ...
    SUBROUTINE GHOST_ZONE_COMM()
        ...
        INTEGER :: reqs(8), rcount
        INTEGER :: mstat(MPI_STATUS_SIZE, 8)
        ...
        rbvals_send(my_jmin:my_jmax,1:nz) = var(my_imax, my_jmin:my_jmax, 1:nz)    
        lbvals_send(my_jmin:my_jmax,1:nz) = var(my_imin, my_jmin:my_jmax, 1:nz)    
        ...
        IF (my_row_rank .gt. 0) THEN  ! rank zero does not receive
            ...
            rcount = rcount+1
            Call MPI_IRecv(lbvals_recv, my_numy*nz, MPI_DOUBLE_PRECISION, source, & 
                rbtag, row_comm, reqs(rcount),ierr)
            var(my_imin-1,my_jmin:my_jmax,:) = lbvals_recv(my_jmin:my_jmax,:)
        ENDIF
        IF (my_row_rank .lt. (npcol-1)) THEN ! highest rank does not send
            ! Send to the right
            dest = my_row_rank+1
            rcount = rcount+1
            Call MPI_ISend(rbvals_send, my_numy*nz, MPI_DOUBLE_PRECISION, dest,rbtag, &
                & row_comm,reqs(rcount),ierr)
        ENDIF
        ...
        IF (my_row_rank .lt. (npcol-1)) THEN ! highest rank does not receive
            ...
            source = my_row_rank+1
            rcount = rcount+1
            Call MPI_IRecv(rbvals_recv, my_numy*nz, MPI_DOUBLE_PRECISION, source, & 
                lbtag, row_comm, reqs(rcount),ierr)
            var(my_imax+1,my_jmin:my_jmax,:) = rbvals_recv(my_jmin:my_jmax,:)
        ENDIF
        IF (my_row_rank .gt. 0) THEN ! rank zero does not send
            ! Send to the left
            dest = my_row_rank-1
            rcount = rcount+1
            Call MPI_ISend(lbvals_send, my_numy*nz, MPI_DOUBLE_PRECISION, dest,lbtag, &
                & row_comm,reqs(rcount),ierr)
        ENDIF
        ...
        ! Copy ghost-zone values into their respective send buffers
        ubvals_send(my_imin:my_imax,1:nz) = var(my_imin:my_imax,my_jmax,1:nz)    
        bbvals_send(my_imin:my_imax,1:nz) = var(my_imin:my_imax,my_jmin,1:nz)    

        IF (my_col_rank .gt. 0) THEN  ! rank zero does not receive
            ! Receive from below
            source = my_col_rank-1
            rcount = rcount+1
            Call MPI_IRecv(bbvals_recv, my_numx*nz, MPI_DOUBLE_PRECISION, source, & 
                bbtag, col_comm, reqs(rcount),ierr)
            var(my_imin:my_imax,my_jmin-1,:) = bbvals_recv(my_imin:my_imax,:)
        ENDIF
        IF (my_col_rank .lt. (nprow-1)) THEN ! highest rank does not send
            ! Send above
            dest = my_col_rank+1
            rcount = rcount+1
            Call MPI_ISend(ubvals_send, my_numx*nz, MPI_DOUBLE_PRECISION, dest,bbtag, &
                & col_comm,reqs(rcount),ierr)
        ENDIF
        ...
        IF (my_col_rank .lt. (nprow-1)) THEN ! highest rank does not receive
            ...
            source = my_col_rank+1
            rcount = rcount+1
            Call MPI_IRecv(ubvals_recv, my_numx*nz, MPI_DOUBLE_PRECISION, source, & 
                lbtag, col_comm, reqs(rcount),ierr)
            var(my_imin:my_imax,my_jmin+1,:) = ubvals_recv(my_imin:my_imax,:)
        ENDIF
        IF (my_col_rank .gt. 0) THEN ! rank zero does not send
            ! Send to below
            dest = my_col_rank-1
            rcount = rcount+1
            Call MPI_ISend(bbvals_send, my_numx*nz, MPI_DOUBLE_PRECISION, dest,lbtag, &
                & col_comm,reqs(rcount),ierr)
        ENDIF
        CALL MPI_Waitall(rcount, reqs(1:rcount), mstat, ierr)
    ...
    SUBROUTINE ALLOCATE_WORKSPACE()
        ...
        ALLOCATE(lbvals_send(my_jmin:my_jmax,1:nz),lbvals_recv(my_jmin:my_jmax,1:nz))
        ALLOCATE(rbvals_send(my_jmin:my_jmax,1:nz),rbvals_recv(my_jmin:my_jmax,1:nz))


        ALLOCATE(bbvals_send(my_imin:my_imax,1:nz), bbvals_recv(my_imin:my_imax,1:nz))
        ALLOCATE(ubvals_send(my_imin:my_imax,1:nz), ubvals_recv(my_imin:my_imax,1:nz))
