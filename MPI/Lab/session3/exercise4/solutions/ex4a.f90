    ...
    SUBROUTINE GHOST_ZONE_COMM()
        ...
        DO k = 2, nz-1
            ...
            IF (my_row_rank .gt. 0) THEN  ! rank zero does not receive
                ...
                rcount = rcount+1
                Call MPI_IRecv(lbvals_recv, my_numy, MPI_DOUBLE_PRECISION, source, & 
            ...
            IF (my_row_rank .lt. (npcol-1)) THEN ! highest rank does not send
                ...
                dest = my_row_rank+1
                rcount = rcount+1
                Call MPI_ISend(rbvals_send, my_numy, MPI_DOUBLE_PRECISION, dest,rbtag, &
            ...
            IF (my_row_rank .lt. (npcol-1)) THEN ! highest rank does not receive
                ...
                source = my_row_rank+1
                rcount = rcount+1
                Call MPI_IRecv(rbvals_recv, my_numy, MPI_DOUBLE_PRECISION, source, & 
            ...
            IF (my_row_rank .gt. 0) THEN ! rank zero does not send
                ...
                dest = my_row_rank-1
                rcount = rcount+1
                Call MPI_ISend(lbvals_send, my_numy, MPI_DOUBLE_PRECISION, dest,lbtag, &
                    & row_comm,reqs(rcount),ierr)
            ENDIF
            ...
            IF (my_col_rank .gt. 0) THEN  ! rank zero does not receive
                ...
                rcount = rcount+1
                Call MPI_IRecv(bbvals_recv, my_numx, MPI_DOUBLE_PRECISION, source, & 
            ...
            IF (my_col_rank .lt. (nprow-1)) THEN ! highest rank does not send
                ...
                dest = my_col_rank+1
                rcount = rcount+1
                Call MPI_ISend(ubvals_send, my_numx, MPI_DOUBLE_PRECISION, dest,bbtag, &
            ...
            IF (my_col_rank .lt. (nprow-1)) THEN ! highest rank does not receive
                ...
                source = my_col_rank+1
                rcount = rcount+1
                Call MPI_IRecv(ubvals_recv, my_numx, MPI_DOUBLE_PRECISION, source, & 
            ...
            IF (my_col_rank .gt. 0) THEN ! rank zero does not send
                ...
                dest = my_col_rank-1
                rcount = rcount+1
                Call MPI_ISend(bbvals_send, my_numx, MPI_DOUBLE_PRECISION, dest,lbtag, &
                    & col_comm,reqs(rcount),ierr)
            ENDIF
